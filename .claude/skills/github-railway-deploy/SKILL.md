...
> **⚠️ MULTIPLE AGENTS SHARE THIS REPO.** Several Claude Code sessions edit the
...
```bash
"$CLAUDE_PROJECT_DIR/.claude/hooks/git-sync.sh" "<type>: <short summary under 72 chars>"
```
...
```bash
"$CLAUDE_PROJECT_DIR/.claude/hooks/git-sync.sh" "feat: add JWT auth endpoints"
```
